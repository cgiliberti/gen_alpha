...
export class TartanScraper extends BaseScraper {
  readonly schoolSlug = 'cmu';
  readonly newspaperUrl = 'https://thetartan.org';
  ...
  async getRecentArticles(): Promise<ScrapedArticle[]> {
    ...
    try {
      const xml = await this.fetchXml('https://thetartan.org/feed/');
      ...
      console.error(`[TartanScraper] Failed:`, err);
    }
