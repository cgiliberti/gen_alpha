...
export class YDNScraper extends BaseScraper {
  readonly schoolSlug = 'yale';
  readonly newspaperUrl = 'https://yaledailynews.com';
  ...
  async getRecentArticles(): Promise<ScrapedArticle[]> {
    ...
    try {
      const xml = await this.fetchXml('https://yaledailynews.com/feed/');
      ...
      $('item').each((_, el) => {
        ...
        const imageUrl = $(el).find('media\\:content, enclosure').attr('url') ?? null;
      ...
      console.error(`[YDNScraper] Failed:`, err);
    }
  ...
  async getArticleFullText(url: string): Promise<string | null> {
    try {
      ...
      return $('div.post-content, .entry-content, .article-content').text().trim() || null;
    } catch {
      return null;
    }
  }
